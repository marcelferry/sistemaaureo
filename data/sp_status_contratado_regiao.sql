CREATE OR REPLACE FUNCTION sp_status_contratado_regiao(
...
DECLARE
    ref refcursor;
BEGIN
  OPEN ref FOR SELECT idregiao, regiao, status, count(*)  
        from (  
            ...
            where  
            	...
            	( pInstituto = 0 OR  i.id =  pInstituto ) and  
            	m.tipo_meta <> 'META_QUANTITATIVA'  
            order by hme.previsao, e.id, r.ciclo, hme.id, hme.tipo_situacao  
          ) relstatus  
